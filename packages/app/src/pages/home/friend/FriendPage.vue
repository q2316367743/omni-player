<template>
  <div class="partner-page">
    <div class="left-panel">
      ...
      <div class="friend-list">
        <div v-if="filteredFriends.length === 0" class="empty-state">
          <p>{{ searchQuery ? '未找到匹配的朋友' : '还没有AI伙伴，快去创建吧~' }}</p>
        </div>
        <div
    ...
    <div class="right-panel">
      <div v-if="selectedPartner" class="friend-detail">
...
watch(() => selectedPartner.value?.id, async (newId) => {
  if (newId) {
    loading.value = true
    fullFriend.value = await useMemoFriendStore().fetchFriend(newId)
    loading.value = false
  }
})
