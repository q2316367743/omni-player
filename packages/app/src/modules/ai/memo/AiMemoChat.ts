...
export async function aiMemoChat(prop: AiMemoChatProp) {
  ...
  logDebug('[AiMemoChat] 查询到上次对话总结', chatSummary ? { summaryId: chatSummary.id, createdAt: chatSummary.created_at } : '无');
...
${formatDate(now)}`;
  ...
  if (chatSummary) {
    const timeSinceSummary = getTimeSinceLastInteraction(chatSummary.created_at);
    messages.push({
      role: "system",
      content: `【上次对话总结】[${timeSinceSummary}]\n${chatSummary.content}`
    });
    logDebug('[AiMemoChat] 添加上次对话总结到消息', { timeSinceSummary });
  }
