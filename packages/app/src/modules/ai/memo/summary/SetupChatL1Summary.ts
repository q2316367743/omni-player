import {
  getMemoChatLastUnSummary,
  listMemoChatUnSummary,
  saveMemoChatSummary,
  updateMemoChat
} from "@/services/memo";
...
export async function setupChatL1Summary() {
  ...
  for (const friend of friends) {
    try {
      // 获取未总结的最后一条
      logDebug('[SetupChatL1Summary-ready] 启动时触发 L1 总结', friend.id)
      const lastChat = await getMemoChatLastUnSummary(friend.id);
      if (!lastChat) continue;
      // 需要满足 时间间隔，最后一条聊天记录需要大于 2 小时
      logDebug('[SetupChatL1Summary-ready] 满足时间间隔条件', friend.id)
      ...
      // 1. 插入总结
      logDebug('[SetupChatL1Summary-ready] 插入总结', friend.id)
      ...
      // 2. 修改消息信息
      logDebug('[SetupChatL1Summary-ready] 修改消息信息', friend.id, messages.length)
      ...
      logError("[SetupChatL1Summary-ready] 总结 L1 信息失败", e);
    }
  }
}
