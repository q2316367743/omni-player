...
export async function listFeed(subscribeId: string, page: number, pageSize: number, keyword?: string): Promise<PageResponse<FeedItem>> {
  const query = useMpSql().query<FeedItem>("feed_item");
...
export async function refreshFeed(subscribeId: string) {
  const feedQuery = useMpSql().query<FeedItem>('feed_item')
  let update = false;
  const subscribeQuery = useMpSql().query<SubscribeItem>('subscribe_item')
  const subscribe = await subscribeQuery.eq('id', subscribeId).one();
  if (!subscribe) return Promise.reject(new Error("订阅不存在"));
  try {
    const feedMapper = useMpSql().mapper<FeedItem>('feed_item')
    ...
    const subscribeMapper = useMpSql().mapper<SubscribeItem>("subscribe_item")
    // 重新计算数量
...
export async function getFeedContent(feedId: string): Promise<FeedWrapper> {
  ...
  const feedContentQuery = useMpSql().query<FeedContent>("feed_content")
  const feedContentMapper = useMpSql().mapper<FeedContent>("feed_content")
