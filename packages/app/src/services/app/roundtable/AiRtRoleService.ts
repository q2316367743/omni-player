import {useAiRtSql} from "@/lib/sql.ts";
import type {AiRtRole, AiRtRoleAdd, AiRtRoleType, AiRtRoleUpdate} from "@/entity/app/ai/roundtable";

export function listAiRtRoleService(type?: AiRtRoleType) {
  return useAiRtSql().query<AiRtRole>('ai_rt_role')
  .eq('type', type).list()
}

export function addAiRtRoleService(prop: AiRtRoleAdd) {
  const now = Date.now();
  return useAiRtSql().mapper<AiRtRole>('ai_rt_role').insert({
    ...prop,
    created_at: now,
    updated_at: now
  })
}

export function updateAiRtRoleService(id: string, prop: Partial<AiRtRoleUpdate>) {
  return useAiRtSql().mapper<AiRtRole>('ai_rt_role').updateById(id, {
    ...prop,
    updated_at: Date.now()
  })
}

export function deleteAiRtRoleService(id: string) {
  return useAiRtSql().mapper<AiRtRole>('ai_rt_role').deleteById(id)
}