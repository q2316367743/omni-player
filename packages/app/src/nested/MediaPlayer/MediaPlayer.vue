<template>
  <t-layout class="player-root">
    ...
    <t-aside width="420px">
      <div v-if="detail"
           class="h-full overflow-auto bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
        <div class="p-4">
          <div class="flex gap-4">
            <div class="w-28 shrink-0">
              <img v-if="detail.posterUrl" :src="detail.posterUrl" :alt="detail.name"
                   class="w-28 h-40 object-cover rounded-2xl shadow-xl"/>
              <div v-else
                   class="w-28 h-40 rounded-2xl bg-gradient-to-br from-blue-500/35 via-purple-500/25 to-cyan-500/35"/>
            </div>
            <div class="min-w-0 flex-1">
              <div class="text-base font-semibold leading-snug">{{ detail.name }}</div>
              <div v-if="detail.originalTitle && detail.originalTitle !== detail.name"
                   class="mt-1 text-xs text-white/65 truncate">{{ detail.originalTitle }}
              </div>
              ...
              <div class="mt-2 flex flex-wrap gap-2">
                ...
                <t-tag v-if="detail.year" theme="default" variant="light" shape="round"
                       class="border-0 bg-black/30 text-white">
                  <span class="tabular-nums">{{ detail.year }}</span>
                </t-tag>
                <t-tag v-if="typeof detail.rating === 'number'" theme="success" variant="light" shape="round"
                       class="border-0 bg-black/30 text-white">
                  <span class="tabular-nums">{{ detail.rating.toFixed(1) }}</span>
                </t-tag>
                <t-tag v-if="detail.runtimeSeconds" theme="default" variant="light" shape="round"
                       class="border-0 bg-black/30 text-white">
                  {{ formatDuration(detail.runtimeSeconds) }}
                </t-tag>
              </div>

              <div v-if="detail.userState?.playbackPositionSeconds && detail.runtimeSeconds"
                   class="mt-3 rounded-xl bg-white/10 p-3 backdrop-blur">
          ...
          <div v-if="detail.genres?.length"
               class="mt-4 rounded-2xl border border-white/10 bg-white/8 p-4 backdrop-blur">
            <div class="text-sm font-semibold">类型</div>
            <div class="mt-2 flex flex-wrap gap-2">
              <t-tag v-for="g in detail.genres" :key="g" theme="primary" variant="light" shape="round"
                     class="border-0 bg-white/12 text-white">
                {{ g }}
              </t-tag>
            </div>
          </div>
              ...
              <div class="mt-3">
                ...
                <div v-if="selectedSeasonId" class="mt-3">
                  ...
                  <div v-else-if="episodeCacheMap[selectedSeasonId]?.length" class="grid grid-cols-4 gap-2">
                    <t-tooltip v-for="(ep, idx) in episodeCacheMap[selectedSeasonId]" :key="ep.id" :content="ep.name"
                               placement="top">
                      <t-button
                      ...
                      >
                        <span class="inline-flex items-center gap-1 min-w-0">
                          ...
                          <span v-if="progressText(ep)" class="text-[10px] text-white/60 truncate">{{
                              progressText(ep)
                            }}</span>
                        </span>
                      </t-button>
...
onMounted(async () => {
  ...
  const {serverId, itemId, mediaId} = props;
