...
export const fetchFileClient = async (id: string) => {
...
export const useFileServerStore = defineStore('file-server', () => {
  ...
  useStore().list(LocalName.LIST_FILE_SERVER).then((res) => {
    servers.value = res;
    console.log("初始化文件服务器列表成功")
  }).catch((e) => MessageUtil.error("初始化文件服务器列表失败", e));
  ...
  const addServer = async (res: FileServerEdit) => {
    ...
    await useStore().save(LocalName.LIST_FILE_SERVER, servers.value);
    // 保存用户名密码
