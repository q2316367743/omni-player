...
import type {MediaEpisode, MediaEpisodeItem} from "@/modules/media/types/detail/MediaEpisode.ts";
...
import {
  ...
  normalizeMediaItem,
  normalizePerson
} from "@/modules/media/services/jellyfin/utils.ts";
...
export class EmbyClient implements IMediaServer {
  ...
  async authenticate(): Promise<void> {
    ...
    if (!username || !password) {
      ...
      throw new Error("Emby username or password is missing");
    }
    ...
    try {
      ...
      if (!this.accessToken || !this.userId) {
        ...
        return Promise.reject(new Error("Emby login response is invalid"))
      }
